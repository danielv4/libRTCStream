...
typedef void (*WuWriteFn)(const uint8_t* data, size_t length, const WuClient* client, void* userData);


typedef enum {
	
	eva_event_binary_data,
	eva_event_client_join,
	eva_event_client_leave,
	eva_event_text_data
} EvaEventType;
...
typedef struct EvaHost EvaHost;
...
const uint8_t kSctpFlagCompleteUnreliable = SctpFlagEndFragment | SctpFlagBeginFragment | SctpFlagUnreliable;
...
enum StunType { 
	Stun_BindingRequest = 0x0001, 
	Stun_SuccessResponse = 0x0101 
};
...
__declspec(dllexport) int32_t eva_host_create(int socket, const char* hostAddr, const char* port, int32_t maxClients, EvaHost** host);
...
__declspec(dllexport) int32_t eva_update(Wu* wu, EvaEvent* evt);

static int32_t WuSendData(Wu* wu, WuClient* client, const uint8_t* data, int32_t length, DataChanProtoIdentifier proto);

__declspec(dllexport) int32_t eva_stream_send_text(Wu* wu, WuClient* client, const char* text, int32_t length);

__declspec(dllexport) int32_t eva_stream_send_binary(Wu* wu, WuClient* client, const uint8_t* data, int32_t length);

__declspec(dllexport) EvaSdp eva_exchange_sdp(Wu* wu, const char* sdp, int32_t length);

void WuSetUserData(Wu* wu, void* userData);

__declspec(dllexport) void eva_handle_datagram(Wu* wu, const EvaAddress* remote, const uint8_t* data, int32_t length);
...
__declspec(dllexport) void eva_srtp_init();

__declspec(dllexport) int eva_send_stun_binding_response(const char *ice_pwd, int ice_pwd_len, int rtpSocket, uint8_t *recvBuffer, int recvResult, struct sockaddr_in clientAddr);

__declspec(dllexport) srtp_t *eva_srtp_create(unsigned char *dtls_buffer);

__declspec(dllexport) int eva_send_rtp_sample(int socket, struct sockaddr_in dst, srtp_t *srtpSession, BYTE *frameData, size_t frameLength, uint32_t ssrc, uint32_t timestamp, uint16_t* seqNum);
